...
| The `bench-mutants` sweep itself (an#124) — *the worst shape of this* | **a different tree from the one it mutated**. Sweeping a copy fixed the concurrency hazard and would have broken the instrument: `cwd=` does not decide where `import an` resolves, so the child would have mutated the copy and tested the real tree — **every mutant surviving, and the sweep reporting that every guard is decoration**. The failure output is indistinguishable from a genuine finding, which no other instance here can claim. `_child_env` pins `PYTHONPATH` to the swept tree; a separate test pins that `_run_one` *uses* it, because the first draft pinned only the helper. |
