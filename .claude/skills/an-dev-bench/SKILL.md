...
| `pix_fmt` | **NOT REGISTERED — it failed its exam** (an#72). Three families on four scenes on macOS/arm64, **none** in CI: family D as-declared on all six there and contrary on three here. **One cause of that is now fixed and it is not the one the issue names** (2026-08-29): `lossless_encode_command` hardcoded `-pix_fmt yuv420p` while the delivered encode reads the rebindable `DEFAULT_PIX_FMT`, so under this lever the reference was a *different colour pipeline* from the delivered file and every metric measured against it silently carried the whole 4:2:0 conversion the reference exists to cancel. The leg tracks the delivered format now (`-crf` must not track, `-pix_fmt` must — it names what libx264 receives). Measured at 4:4:4 on the ten-scene corpus, coupling changes the **sign** of family E on three scenes and moves family D by up to 18 percentage points, so any pre-2026-08-29 `pix_fmt` number was taken across that boundary and should not be reused. **The contamination was exactly the RGB-domain metrics** — `flat_field_deviation` and `encode_flicker_on_held_pixels` reduce over the three channels, so chroma reaches them; `coded_luma_edge_error` is bit-identical between a 4:2:0 and a 4:4:4 leg on all ten scenes, i.e. swscale's luma is format-independent and the luma metrics were never affected. **What coupling does NOT fix, and cannot**: the lever's benefit lives *in the conversion*, and lossless-referencing exists to cancel the conversion — so no lossless-referenced metric can witness it, and `flat_field_deviation` still moves contrary on `graded_field` (+84.2%) and `prop_swap` (+127.4%) with the leg coupled. **Family C still cannot supply a witness**: `chroma_edge_dCr` is the lever's headline (-3.8% to -92.2%, 9 of 10 scenes) and references `source_png`, disqualified as build-dependent by `test_every_counting_encode_metric_references_the_lossless_leg`. That disqualification rests on a **luma** measurement (`png_to_encoder_input_luma`, 0.0 here / 0.629 on the Linux runner) and there has never been a chroma one — measured 2026-08-29 against a 4:4:4 leg it is 0.0000 / max 0 on all ten scenes here. **That number is no longer the question, though**: over 20 build variants (8 downsampling × 7 upsampling kernels × 5 siting conventions) the 4:2:0 reading moves +10.6% to +105.5% while the 4:4:4 reading is byte-identically constant, and the **sign** of (420 − 444) flips on 5 of 10 scenes. So the chroma witness is inadmissible on sign-robustness grounds whatever the Linux residual turns out to be. **Two further defects the issue does not name.** (a) At fixed crf this is **not a single-variable lever**: 4:4:4 spends *fewer* bits (`video_stream_bytes` −2 to −7% on 9 of 10 scenes) at a **coarser** quantiser — x264's own I-frame Avg QP rises +2.9 to +9.5 (`saturated_outline` 11.26 → 18.99) — because CRF is a rate factor, not a QP. A lever must be one variable. (b) **`flat_field_deviation` is a hard-threshold counter sitting on the crossover of those two opposed effects**, and that, not build variance, is what the exam actually tripped over. Measured 2026-08-29 on a 5 × 3 grid of its own two free parameters (`tol` 4–8 × `FLAT_DILATE_K` 1/3/5), both formats rendered with the leg tracking so the reference is not a variable: **3 of 6 scenes are sign-stable across all fifteen combinations and 3 flip** — `graded_field` reads `increase` at tol 4–7 and `decrease` at tol 8 at every k (+84.2% → −81.6% at k=3); `promote_demo` flips with both parameters; `aa_probe` flips only at (k=5, tol=4). That is the **same 3-stable/3-fragile split the exam saw between macOS and Linux**, reproduced on ONE machine and ONE x264 build by nudging the metric's own knobs. Margins at the shipped (tol=6, k=3): `graded_field` **+449 counted pixels of 429,623** (0.10% of the flat mask), `prop_swap` +1,506 of 894,780. `flat_mask` erodes exactly where 4:2:0 does its damage (at k≥5 the 444 counts collapse to 0 on four of six scenes), so what it counts is a one-pixel fringe. **A metric whose mask deletes the lever's subject cannot carry a stable prediction about it**, and this is a defect in family D that will bite the next lever landing near its threshold. The one route the evidence leaves open, unproposed so far: make the lever single-variable by matching the **quantiser** rather than the CRF. Also the lever that shows family A is blind to the encoder BY MEASUREMENT: exactly +0.0% on every family-A metric, all six scenes |
