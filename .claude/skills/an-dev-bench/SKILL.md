...
| `pix_fmt` | **NOT REGISTERED — it failed its exam** (an#72), and the 2026-08-29 investigation makes the case *stronger*, not weaker. Original result: three families on four scenes on **macOS/arm64**, none on **Linux/x86-64** — family D as-declared on all six on macOS and contrary on three on Linux. **Two defects were fixed and neither is the one the issue names, and a third — the real one — is not fixed.** (1) `lossless_encode_command` hardcoded `-pix_fmt yuv420p` while the delivered encode resolves at call time, so under a 4:4:4 delivery the reference was a different colour pipeline from the file it referenced. The leg is now **probed off the delivered mp4** — note there are TWO seams that set the format (`RenderContext.pix_fmt`, which `an render --pix-fmt` uses and which the mux resolves FIRST, and the `DEFAULT_PIX_FMT` global, which is only its fallback and which NO registered lever rebinds — `MUTATIONS` is `high_crf`/`disabled_aa`/`supersample`, enforced at import), and a leg that consults either one covers only that one. Contamination reached exactly the RGB-domain metrics (`flat_field_deviation`, `flat_field_p99_dev`, `encode_flicker_on_held_pixels`); `coded_luma_edge_error` and `encode_ringing_excess` are identical to the last digit between a 4:2:0 and a 4:4:4 leg on all ten scenes, so swscale's luma is format-independent. Coupling changes family E's sign on `aa_probe`/`multi_shot`/`saturated_outline` **from as-declared to contrary** (7/10 as-declared before, 4/10 after) and moves family D by up to **24.8 pp** (`prop_swap`; 18.0 pp over the original six). **So any pre-2026-08-29 `pix_fmt` number was taken across that boundary, and the fix makes the lever's evidence weaker.** (2) At fixed crf this is **not a single-variable lever**: 4:4:4 spends *fewer* bits (`video_stream_bytes` −1.99% to −7.63% on 9 of 10 scenes; `single_character` +0.22% is the exception) at a **coarser** quantiser — x264's own I-frame Avg QP rises +2.9 to +9.5 (`saturated_outline` 11.26 → 18.99) — because CRF is a rate factor, not a QP. A lever must be one variable. (3) **The real one: `flat_field_deviation` is a hard-threshold counter sitting on the crossover of those two opposed effects.** Measured on a 5 × 3 grid of its own free parameters (`tol` 4–8 × `FLAT_DILATE_K` 1/3/5), both formats rendered with the leg tracking so the reference is not a variable: **3 of 6 scenes sign-stable across all fifteen combinations, 3 flip** — `graded_field` reads `increase` at tol 4–7 and `decrease` at tol 8 at every k (+84.2% → −81.6% at k=3); `promote_demo` flips with both parameters; `aa_probe` only at (k=5, tol=4). **That is the same 3-stable/3-fragile split the exam saw between macOS and Linux, reproduced on ONE machine and ONE x264 build by nudging the metric's own knobs** — so the cross-machine difference was a symptom, not the cause. Margins at the shipped (tol=6, k=3): `graded_field` **+449 counted pixels of 429,623** (0.10% of the flat mask). `flat_mask` erodes precisely where 4:2:0 does its damage, so what it counts is a narrow fringe. **A metric whose mask deletes the lever's subject cannot carry a stable prediction about it**, and this will bite the next lever landing near its threshold. **Family C still cannot supply a witness**: `chroma_edge_dCr` is the lever's headline (−3.8% to −92.2%, 9 of 10 scenes) and references `source_png`, disqualified by `test_every_counting_encode_metric_references_the_lossless_leg` — whose quantifier is `any(...)` over ALL mutations, so counting it under `pix_fmt` alone still trips it. That disqualification rests on a **luma** build-residual (`png_to_encoder_input_luma`, 0.0 here / 0.629 on the Linux runner); the chroma analogue measured against a 4:4:4 leg is 0.0000 / max 0 on all ten scenes here, and the Linux number is unknown. Its DIRECTION is fairly robust — across 8 swscale downsampler kernels the sign of (420 − 444) flips on only **1 of 10** scenes (`prop_swap`, where the effect is smallest) — but the MAGNITUDE moves **0.2% to 56.4%**, which is what a machine-scoped counting value cannot carry. (Beware: `-sws_flags` before `-i` is an INPUT option and changes nothing; the kernel has to be set on an explicit `scale` filter, and a first attempt that got this wrong produced nine byte-identical 'variants'.) **The route nobody has proposed**: make the lever single-variable by matching the **quantiser** rather than the CRF, which removes (2)'s opposing term and lets the chroma effect stand against a family D no longer on a crossover. Also the lever that shows family A is blind to the encoder BY MEASUREMENT: exactly +0.0% on every family-A metric, all six scenes |
| `flat_camera` | **NOT REGISTERED — refused at comparability, on the recorded `step_hz` verdict** (an#111). A flattened-parallax mutant is a **compile-time** change: it rewrites the plane-compensation channels, so it moves `scene_contract_sha256` and `bench-compare` refuses the row **before any family is examined** — the same wall `step_hz` hit, and unlike `pix_fmt`, which stayed comparable and *failed* its exam. So the parallax claim is not carried by a lever at all. It is carried **render-side**, by the `stage_min_plane_ratio_gap` diagnostic row (family B, `role="diagnostic"`, guard optimum, counts zero, gated under both edge levers) and the `stage_planes_parallaxed` tripwire, floor at **half the first bless's measured 0.375** — the `expression_min_pairwise_changed_px` precedent, followed literally. The proof that the guards work belongs in `an bench-mutants` as declared guard mutants, not here. Full reasoning: `an-dev-stage` §3 |
...
**The corpus is ten scenes, not the six this table exercises.** Wave 7 added two
that no lever reaches and that exist for the render-side instruments instead:
`prop_swap` (an#108 — a two-state prop, blessed in an#111) and `stage_pan`
(an#111 — three depths under a pan, goldens pinned at frames 0 and 8, which is
where all three planes land on exact pixel boundaries; anywhere else the
anti-aliased edge changes each mask's size and a centroid is measured against a
different shape). If you are adding a lever, the question "which scene can this
reach" now has ten candidates and the answer is still usually one of the six.
