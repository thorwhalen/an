...
- ` ```dialogue ` — `speaker [emotion]: text` per line. Emotion is an expression preset — `neutral | happy | sad | angry | surprised | afraid | disgusted | thinking | skeptical | amused` (an unknown name is a validate ERROR) and drives eyebrow tilt during the line.
