...
| Stepped timing, opt-in — `Meta.step_hz` / `Shot.step_hz` (shot wins; `an render --step-hz N`): authored tweens resampled onto a scene-wide pose grid of step-eased keyframes (15 at 30 fps = "on twos", 10 = "on threes"); camera, blinks, `play` clips and swap channels exempt **by construction** (separate emission sites); stamped into `meta.step_hz` only when set, so an unstepped scene's contract hash is unchanged. **No bench lever**: a render-side lever can count at most families A+B+F and stepping has no direction on A, so it cannot pass the three-family exam (the `pix_fmt` precedent) — the human side-by-side is the `stepped-timing` demo, and the default stays smooth (an#89) | `an/adapters/cutout/compile.py` — `step_times`, `_stepped_keyframes`; `an/ir/schema.py` | shipped (an#89) |
