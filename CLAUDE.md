...
- **`gaze_x` / `gaze_y` are accepted and bind to nothing**: the axes exist and the binding looks for `left_pupil` / `right_pupil` slots, which no rig has until PR-D (#99) ships the eye stack and `an character add-gaze`. Validate accepts them; the compiler emits nothing for them.
