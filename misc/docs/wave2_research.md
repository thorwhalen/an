...
1. **Does SwiftShader produce identical pixels across CPU architectures?** Everything here is one arm64 Mac; both cross-build tests used the same `SwiftShader Device (LLVM 10.0.0)`. *Settled by:* one CI run of the corpus on an x86-64 Linux runner with the §2 pins, diffing decoded pixels against the Mac capture. Blocked on an#22. Until it exists, the ledger is machine-scoped and the mutation test must assert **direction of movement against a same-machine control render**, not equality against a committed absolute.
