...
(`compile.py:2905`) maps `shot.camera.move` through `_CAMERA_MOVES` (`:2897-2903` — five entries,
...
`applyProperty` (`runtime.js:456-497`) and in `RUNTIME_APPLIED_PROPERTIES` (`compile.py:272-282`).
...
every `scene.md` camera block carries eight junk lines.
...
zoom, not a pan. **Zero environment documents exist on disk anywhere in the repo** (verified: the
five `examples/*/assets/environments/` directories are empty). And `_build_scene_root` runs
environments and characters in **two separate loops** (`:701-716`), so re-ordering `entities`
cannot interleave them: a plane in front of the characters is not merely absent, it is
...
(`validate.py:114`) and skipped by the compiler (`compile.py:724-726`), so a scene declaring one
...
disagreeing** five-entry palette table at `factory.py:546-552`).
...
Godot states our form verbatim (`ParallaxLayer.motion_scale`: `position = screen_offset·(1 −
scroll_scale)`); After Effects states the size half in one sentence (*"a layer that is the Zoom
...
the document they produce today** — a two-keyframe `ease_in_out` scale tween on `root` and *no*
pivot channels — or every committed camera scene's contract hash moves. New in 7a: `pan_left`,
...
the evaluators are later-wins with camera clips appended last — so the failure would be *silent*.
The stage emitter must **detect** an authored `set`/`tween` on `(node, x|y)` for any node
carrying a non-unity factor and **raise**, naming both. Additively folding the two (the way
`_add_face_clips` sums contributors) is the eventual right answer and is **not Wave 7**.
...
today (§1), and `.claude/skills/an/SKILL.md:54` claims unknown environment keys *raise* when the
compiler only warns — so the one place a reader would check is wrong in the safe-sounding
direction.
...
- **"a prop is a character with `kind: prop`"** — `CharacterDescriptor.model_post_init`
  (`schema.py:369-384`) re-seeds a seven-bone humanoid with a face and a blink from an empty
  ...
  blocking findings on a correct prop; and five call sites already branch on kind.
...
get: assert **`Δy ≈ 0`** (so "it panned" is distinguishable from "it zoomed"), and assert the
**ordering** `p_far < p_mid < p_near` — planes moving at different rates in the *wrong* order is
a real bug a bare inequality passes.
...
every plane local `Δx = 0`. This needs `_python_timeline` promoted out of
`tests/test_swap_channels.py:64-108` into `an/adapters/cutout/timeline.py` (zero behaviour
change).

**Measurement (b), pixels, labelled PR:** a `stage_pan` corpus fixture with three distinctly
coloured planes; per-plane centroid displacement between goldens via exact-colour masks
...
1. **`Shot.style` → `Shot.renderer`** + the `kind="style"` retirement, one migration. The only
   ...
   asserted equal to the committed row.
2. **Promote `_python_timeline`** into `an/adapters/cutout/timeline.py`.
...
Krita `.kpl`; WHATWG/MDN `createImageBitmap` premultiply and colour-space options. Each thread's
raw notes record the exact URL and the quoted sentence; anything unfetchable is marked UNVERIFIED
there and is not relied on here.
