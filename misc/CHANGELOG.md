...
=== TESTS ===
- tests/test_cutout_runtime_files.py::test_the_capture_page_never_stops_compositing_the_stage_canvas
  asserts: No `#stage` rule inside `index.html`'s `<style>` block contains any of `display:none`, `visibility:hidden`, `content-visibility:hidden`, `opacity:0` (both spaced and unspaced spellings). Runs in the default lane — no browser, no ffmpeg.
  mutation: In `an/data/cutout_runtime/index.html`, change `#stage { display: block; }` to `#stage { display: none; }` — i.e. apply an#57 part 1 exactly as written. Declared as the `capture_page_stops_compositing_the_canvas` mutant in `an/bench/mutants.py` so `an bench-mutants` re-runs the proof. Also verify the negative direction: `#stage { display: block; }` must leave it green, and an unrelated `display:none` on some other selector in the same style block must NOT fail it (the assertion is scoped to lines containing `#stage`).
- tests/test_cutout_render.py::test_hiding_the_stage_canvas_breaks_element_capture
  asserts: With the real runtime loaded in headless Chromium at 320x240: (1) after `style.display='none'`, `page.locator('#stage').is_visible()` is False and `Locator.screenshot(timeout=2000)` raises with 'imeout' in the message; (2) after `style.opacity='0'`, `is_visible()` is True — Playwright accepts it — yet the returned PNG bytes differ from the baseline (measured: it is uniform white, 1 distinct RGBA value, against a baseline with >1). Marked `@pytest.mark.browser` only.
  mutation: This test IS the measurement, so its mutation is a change to the thing measured, not to `an`: point `_capture_frames` at an in-page `toDataURL` capture. Under that capture path the `display:none` leg would stop being a refusal, and the test's premise — and therefore the static guard it justifies — must be revisited in the same PR. To prove the test is not vacuous today, invert each assertion in turn: `assert page.locator('#stage').is_visible() is True` after `display:none` must fail, and `assert hidden == baseline` after `opacity:0` must fail.
- tests/test_encode_pins.py::test_every_command_that_writes_an_mp4_asks_for_faststart
  asserts: Intercepting `subprocess.run` in BOTH `an.adapters.cutout.render` and `an.render`, all three commands — `_ffmpeg_mux`, `_ffmpeg_add_audio`, `_ffmpeg_concat` (2 inputs) — contain the adjacent pair `('-movflags', '+faststart')`. No ffmpeg binary needed; runs in the default CI leg.
  mutation: Delete `*MP4_FASTSTART_ARGS,` from `an/adapters/cutout/render.py::_ffmpeg_add_audio` — this is the state of `main` today, so the test must be RED before the fix and green after. Then repeat for `an/render.py::_ffmpeg_concat` (also red on main) and for `_ffmpeg_mux` (green on main — this leg is the one that already passed, which is why it alone was never sufficient). Each deletion must fail the test naming the specific command, not just 'faststart missing'.
- tests/test_encode_pins.py::test_the_delivered_mp4_puts_moov_before_mdat
  asserts: After a real encode chain (4 flat PNGs → `_ffmpeg_mux` → `_ffmpeg_add_audio` → `_ffmpeg_concat` twice, once with one input and once with two), both the single-shot and the concatenated mp4 have their top-level `moov` atom at a lower byte offset than `mdat`. Atom table walked with `struct` alone, no mp4 library. `@pytest.mark.ffmpeg`.
  mutation: Delete `*MP4_FASTSTART_ARGS,` from `_ffmpeg_add_audio` — BOTH legs must go red (this is the state of `main`, verified against committed output: `examples/single_character/output/main.mp4` and `examples/park_bench_cartoon/output/main.mp4` are both `ftyp free mdat moov`). Then, separately, delete it from `_ffmpeg_concat` only — the multi-shot leg must go red while the single-shot leg stays green, which is what proves the two legs are not redundant. This is the guard the command-level test cannot be: `_ffmpeg_mux` passed the flag for the whole life of the project and no deliverable ever carried it.
- tests/test_encode_pins.py::test_faststart_on_the_concat_is_a_remux_not_a_re_encode
  asserts: The concat output's H.264 elementary stream (extracted with `-c copy -bsf:v h264_mp4toannexb -f h264`) is byte-identical to the two inputs' elementary streams concatenated. `@pytest.mark.ffmpeg`.
  mutation: In `an/render.py::_ffmpeg_concat`, change `"-c", "copy",` to `"-c:v", "libx264",` — a real transcode. The extracted stream then differs and the test must fail with the message pointing at the double-encode risk an#57 named. A weaker mutation that must ALSO fail it: change the concat inputs' order (`[b_mp4, a_mp4]`), proving the assertion is on ordered bytes and not on a length or a set.
- tests/test_encode_pins.py::test_faststart_moves_no_number_the_bench_reads (optional, recommended)
  asserts: For one real encode chain run with and without `MP4_FASTSTART_ARGS` on `_ffmpeg_add_audio`: `Path.stat().st_size` is equal, `an.bench.imageio.video_stream_bytes` is equal, and the decoded YUV (`an.bench.imageio.decoded_yuv_command` piped to bytes) is sha256-identical. `@pytest.mark.ffmpeg`.
  mutation: Change `_ffmpeg_add_audio`'s `"-c:v", "copy"` to `"-c:v", "libx264"`. All three assertions must break. This is the test that discharges 'no re-bless is owed' as a measured claim rather than a sentence in the changelog — it is the reason `MP4_FASTSTART_ARGS` may stay out of `DETERMINISTIC_X264_ARGS`, which is an `ENCODE_ENV_PATHS` comparability key.

=== RISKS ===
- **The map changes `an/data/cutout_runtime/index.html`, which moves `runtime_sha256`.** That is provenance only, NOT a comparability key — stated explicitly at `an/bench/environment.py:118-127` and confirmed by `MUTATION_TOUCHES` at `an/bench/registry.py:114-122`, where it is the `disabled_aa` lever's fingerprint. So no ledger row becomes incomparable. But it DOES mean `_verify_disabled_aa` compares against a new pristine digest — that is by design (`"Compared against the pristine runtime rather than a stored constant, so a legitimate change to runtime.js does not turn this into a re-baselining chore"`), so nothing breaks. Do not let a reviewer 'fix' this by pinning the digest.
- **The `disabled_aa` lever string-matches `antialias: true` and refuses unless it finds EXACTLY ONE** (`an/bench/mutations.py:153-163`). My index.html comment must not contain that literal. It does not. Any reviewer expanding the comment must keep it out.
- **`_capture_frames`'s signature is called by keyword from a test.** `tests/test_loud_discards.py:593` calls `_capture_frames(_ThrowingPage(), total_frames=3, fps=12, frames_dir=Path("/tmp"))`. If a later in-page-capture PR changes that signature (adding e.g. a `capture=` seam), that test breaks. Nothing in Part 1 or Part 2 as mapped touches it.
- **Five `+faststart` sites exist; the map touches three.** `an/adapters/cutout/render.py:517` (mux), `_ffmpeg_add_audio` (adding), `an/render.py` `_ffmpeg_concat` (adding), plus **`an/characters/record.py:146-147`** (the webm→mp4 recorder — a fourth, independent x264 site that the skill's §3 already flags) and **`an/bench/imageio.py:184-185`** (`lossless_encode_command`). Leave the last one alone: it binds `DETERMINISTIC_X264_ARGS` at IMPORT time precisely so a lever cannot move the lossless reference, and touching it risks that invariant for zero benefit.
- **The command-level faststart test monkeypatches `subprocess.run` in TWO modules.** `an/adapters/cutout/render.py` and `an/render.py` each do `import subprocess` separately, so patching one does not reach the other — and it also patches `an.render.shutil.which`, because `_ffmpeg_concat` raises `RenderError` before building the command when ffmpeg is absent. Miss either and the test passes vacuously on a machine with ffmpeg and fails confusingly on one without.
- **`test_the_delivered_mp4_puts_moov_before_mdat` is only sound because moov size is stable.** ffmpeg's faststart pass rewrites chunk offsets in place; if they outgrew 32 bits it would promote `stco` to `co64` and the file size would change. Measured here it does not (moov 1062 bytes both ways, file 4920 both ways, and 13772 both ways on real `an` output). For any file `an` produces this holds, but do not write a docstring claiming it universally.
- **The two ffmpeg-marked tests do not run in the default CI leg.** Per the repo's standing rule, do not write that faststart is "verified in CI" — say which lane. The command-level test does run everywhere; the atom-order and remux tests need `AN_BROWSER_TESTS=1` locally or the `run-browser-tests` label on the PR (`gh api -X POST repos/thorwhalen/an/issues/<N>/labels -f 'labels[]=run-browser-tests'`; `gh pr edit --add-label` silently no-ops on this repo). **Part 2 does not move a pixel** — measured, decoded YUV sha256 identical — but Part 1's browser test does need the label.
- **A drive-by doc drift I did not map:** `tests/test_preview_reload.py:12` names the static guard `test_load_scene_does_not_detach_the_canvas`, but the test in `tests/test_cutout_runtime_files.py:34` is actually `test_load_scene_installs_a_fresh_canvas_on_reload`. I am adding a sibling guard to that file; whoever lands this could fix the cross-reference in the same pass.
- **All timings are single-machine (macOS arm64, Chromium 140.0.7339.16 via the pinned playwright==1.55.0) and were taken on a synthetic 24-rect 1080p scene, not the corpus.** The ratios are large and reproduced across interleaved rounds (A 115.87 / B 31.50 / C 34.31 ms/f, n=36 each, p10–p90 tight), but the absolute numbers should not be quoted as corpus figures.
- **ffmpeg 8.1 (Homebrew) is not necessarily CI's build.** The remux verdict is a property of the build, which is exactly why `test_faststart_on_the_concat_is_a_remux_not_a_re_encode` asserts it rather than a comment claiming it. On a build where the flag forced a transcode, that test goes red with an actionable message instead of the corpus silently acquiring a second encode.

=== OPEN ===
- **Part 1 is not landable as an#57 specifies it — the user has to choose.** (a) Close the `display:none` half as refuted and fold the measurement into whatever issue owns in-page capture (Wave 2 measured that at 3.5x; I measure 3.4x, and the hiding adds 1.09x on top). (b) Land the combined change — in-page `toDataURL` capture + `display:none` — as one PR with a golden-corpus pixel gate. My RGBA byte-identity result is one synthetic scene at one time on one machine; the repo's standard for "the pixels did not move" is the 12 committed goldens on decoded pixels, and that has not been run. (c) Leave the issue open with the map recorded. I have mapped (c)+documentation; (a) and (b) both need a ruling.
- **`encode_command_source` is now known to be inaccurate and I deliberately did not fix it.** `an/bench/run.py:625-627` records `"an.adapters.cutout.render._ffmpeg_mux + DETERMINISTIC_X264_ARGS"`, but the delivered mp4 also passes `_ffmpeg_add_audio` and, for multi-shot, `_ffmpeg_concat`. It is an `ENCODE_ENV_PATHS` comparability key (`an/bench/compare.py:99`), so correcting the string refuses **every encode-side metric against every existing ledger row** — for a change measured to move none of them. Suggested: correct it in the PR that next legitimately re-baselines encode-side numbers (PR-5, `-pix_fmt`), not this one. Needs a ruling because leaving a knowingly-wrong provenance string is also a cost.
- **Should `_ffmpeg_mux` keep `+faststart` at all?** After the fix it applies only to a discarded intermediate, and a flag on a file nobody receives is precisely the shape that made this bug survive. Removing it saves one container-rewrite pass per shot and changes no delivered byte. I mapped *keeping* it (routed through the constant) because `tests/test_encode_pins.py::test_the_mux_command_carries_the_pins` and the `high_crf` lever both read that command and removing a flag from a tested seam is its own decision.
- **The 100 ms/frame element screenshot is the largest unattributed cost in the frame path** and is not any known issue. `toDataURL` on the same canvas is 10 ms. Playwright's `_preparePageForScreenshot` runs `safeNonStallingEvaluateInAllFrames` plus `await document.fonts.ready` on every call (`screenshotter.js:212-226`). Worth filing? It bounds every render-speed conversation in Wave 3, including whether k=2's 2.54x matters.
- **Should `misc/docs/wave3_research.md` be edited at all?** The handoff calls it FACT and it is dated 2026-08-22. I mapped a strike-through + SETTLED edit to §7 following `wave2_research.md`'s own precedent, but if the rule is "a dated record is immutable", the alternative is to leave it and carry the settlement only in the skill and the issue.
