...
- **Wave 5 PR-D: `play` resolves a descriptor animation into channels** (an#7, closes it). `PlayAction.target` names the entity → its migrated descriptor → `animations[name]` (the seeded `idle_breath` and `blink`, or anything an author adds) → tracks converted to channels on the entity's built nodes, one clip **per play instance** (`__play__{n}`) so two plays of one animation cannot share loop/speed. Two conversions the naive version got wrong, both measured and pinned: bone tracks are DEVIATIONS in view-box units (rotation in degrees) around the bone's rest, and channels carry ABSOLUTE scene values (radians) — so every value is `rest + deviation·k`, read off the built node's transform (a naive copy teleported the torso to y≈±2 and threw on `rotation_deg`); slot tracks name ATTACHMENTS, and swap channels carry set KEYS — each frame resolves through the node's projection, the same swap path an authored `set` rides. Sine tracks are sampled at the frame rate; `loop` is the action's override or the animation's own (`PlayAction.loop` is now `bool | None`). The three refusals that stood in for the feature are gone: scene.md parses `{kind: play, target, animation, [duration], [speed], [loop], [start]}` (the writer had emitted that shape all along — the pair was asymmetric), `an validate` checks the name against the (migrated) descriptor's animations, and the compiler's re-pass is a defensive check that no longer fabricates or refuses by policy. Closes the CLAUDE.md gap "nothing ever emits a non-default `loop_mode`" (`idle_breath` compiles to `loop`). `PlacedClipJSON.loop_mode` (issue #7's step 2) stays deferred until clip dedup exists — per-instance clips make it unnecessary today.
