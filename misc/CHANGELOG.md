...
- **Wave 5 PR-E: `step_hz` — opt-in stepped timing for authored tweens, camera exempt by construction** (an#89). Typed additive fields `Meta.step_hz` / `Shot.step_hz` (shot wins; `None` = smooth), carried through `scene.md` in **both** directions (the writer enumerates meta keys and the reader whitelists shot keys, so each needed its own edit), validated `0 < step_hz <= fps`, threaded `RenderContext.step_hz` → `compile_shot(step_hz=)` (`an render --step-hz N`, `preview` too). The compiler resamples each tween's curve onto a **scene-wide** pose grid (`step_times`: multiples of `1/step_hz` on the shot's clock, plus the tween's own start and end) of step-eased keyframes whose values are the smooth curve's at those instants, evaluated through the Python spec; 15 at 30 fps is "on twos", 10 "on threes". The camera, compiled blinks, `play` clips and swap channels are untouched because they are separate emission sites, not string-sniffed. Stamped into `meta.step_hz` **only when set** (a wrap serializer drops the `None`), so an unstepped scene's compiled document — and every committed ledger row's `scene_contract_sha256` — is byte-identical to before the field existed; recorded per shot in render and bench provenance. **No bench lever, on measurement** (the `pix_fmt` precedent): forced `step_hz=15` over the corpus leaves the two tween-less scenes bit-identical and moves every family on the other four *with the content* and in both directions — the instrument measures per-frame quality against references that moved, so no counting family has a direction and a registered mutation could not pass the exam; the numbers are in the `an-dev-bench` skill's lever table. The human side-by-side is the `stepped-timing` demo (smooth ∥ stepped, 6 Hz because a 12 fps GIF cannot show 12 Hz). **The default stays smooth**; a flip is a separate one-line PR on a human verdict, since the ledger has nothing to add to it.
