...
def test_a_golden_blessed_at_another_resolution_does_not_crash_the_row(tmp_path):
    ...
    _bless(capture, [0.0, 0.25], tmp_path)
    # The mismatch is produced the way the product produces it — the SCENE's
    # resolution changed — rather than by rewriting the committed goldens in
    # place. Hand-editing a golden is now its own, different failure
    # (`record_disagrees`): the file stops being the picture that was blessed,
    # which is not what this test is about.
    reshaped = _fake_capture(tmp_path / "reshaped", size=(9, 5))

    result = G.compare_scene(
        reshaped, times=[0.0, 0.25], chromium_build="99.0.0.0", root=tmp_path
    )
    assert result["state"] == "measured"
...
def test_a_shape_mismatch_reports_no_pixel_count_rather_than_zero(tmp_path):
    ...
    capture = _fake_capture(tmp_path, size=(8, 6))
    _bless(capture, [0.0, 0.25], tmp_path)
    # Same shape mismatch, produced the way the product produces one: the
    # scene's resolution moved. Rewriting a committed golden in place is now
    # `record_disagrees` — a different failure with a different message.
    reshaped = _fake_capture(tmp_path / "reshaped", size=(9, 5))
    result = G.compare_scene(
        reshaped, times=[0.0, 0.25], chromium_build="99.0.0.0", root=tmp_path
    )
...
# ------------- an#54: the committed PNG and its own bless record must agree


def test_a_golden_whose_pixels_disown_its_bless_record_is_unavailable(tmp_path):
    """MUTATION: `if expected is not None and expected != ...:` -> `if False:`.

    `golden_sha256` and the bless record's `frames[].sha256` are the same digest
    of the same file, written by two different calls, and nothing checked they
    still agree. A disagreement means the committed PNG is not the picture a
    human blessed — an edited file, a half-finished re-bless, a frame
    regenerated by another tool — and every one of those read as a clean PASS.

    `unavailable`, not a new gate: a check that could not run is not evidence
    that anything is fine.
    """
    capture = _fake_capture(tmp_path, size=(8, 6))
    _bless(capture, [0.0, 0.25], tmp_path)

    # Edit one committed golden IN PLACE, at the same size, leaving the record
    # untouched. This is what a hand-edit looks like from the gate's side.
    victim = sorted(G.iter_committed("scene", "99.0.0.0", root=tmp_path))[0]
    pixels = png.read_png(victim)
    pixels[0, 0] = 255 - pixels[0, 0]
    png.write_png(victim, pixels)

    result = G.compare_scene(
        capture, times=[0.0, 0.25], chromium_build="99.0.0.0", root=tmp_path
    )
    assert result["state"] == "unavailable"
    assert "disagree" in result["detail"] and "bless record" in result["detail"]


def test_an_absent_bless_record_gates_nothing(tmp_path):
    """Absent is unknown, not different — the rule `an.bench.compare` applies too.

    MUTATION: treat a missing record as a disagreement. Every scene blessed
    before this check existed would then read `unavailable` forever.
    """
    capture = _fake_capture(tmp_path, size=(8, 6))
    _bless(capture, [0.0, 0.25], tmp_path)
    G.manifest_path("scene", "99.0.0.0", root=tmp_path).unlink()

    result = G.compare_scene(
        capture, times=[0.0, 0.25], chromium_build="99.0.0.0", root=tmp_path
    )
    assert result["state"] == "measured"
    assert all(f["bless_record_sha256"] is None for f in result["frames"])


def test_the_cross_check_is_on_pixels_so_a_re_encode_is_invisible(tmp_path):
    """A different zlib level changes every file byte and zero pixels.

    Chromium 1187 -> 1223 did exactly that to 144 of 144 files. If the
    cross-check compared FILE bytes, that bump would have read as 144 golden
    frames disowned by their records.
    """
    capture = _fake_capture(tmp_path, size=(8, 6))
    _bless(capture, [0.0, 0.25], tmp_path)
    for path in G.iter_committed("scene", "99.0.0.0", root=tmp_path):
        png.write_png(path, png.read_png(path), level=1)
...
def test_the_committed_goldens_all_agree_with_their_own_records():
    """Runs against the REAL corpus, not a fixture. The done-when's companion."""
    from pathlib import Path as _P

    from an.bench.golden import load_manifest
    from an.bench.paths import golden_dir

    root = _P(__file__).resolve().parents[1]
    scenes = sorted(p.name for p in golden_dir(root).iterdir() if p.is_dir())
    assert scenes, "the committed golden corpus is missing"
    checked = 0
    for scene in scenes:
        for record_path in (golden_dir(root) / scene).glob("bless-chromium*.json"):
            build = record_path.stem[len("bless-chromium") :]
            record = load_manifest(scene, build, root=root)
            for frame in record["frames"]:
                path = golden_dir(root) / scene / frame["file"]
                assert G.pixels_sha256(png.read_png(path)) == frame["sha256"], (
