...
def test_the_passes_thin_the_raw_track_but_not_below_the_old_condenser(monkeypatch):
    ...
    before = _evaluate(timeline_from_scene(off), 14 / 24)[("talker/head/mouth", "viseme")]
    after = _evaluate(timeline_from_scene(on), 14 / 24)[("talker/head/mouth", "viseme")]
    assert before == "C"
    assert after != before
...
def test_out_of_order_dialogue_lines_are_emitted_in_time_order():
    ...
    for order in ([line1, line2], [line2, line1]):
        ...
        js = to_dict(compiled)
        assert _evaluate(timeline_from_scene(compiled), 18 / 24)[("c/head/mouth", "viseme")] == "A", order
