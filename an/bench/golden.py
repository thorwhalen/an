...
def compare_scene(
    ...
    *,
    ...
    # The bless record beside the frames carries the SAME digest as the frames
    # themselves — `bless_scene` hashes what it wrote, this function hashes what
    # it reads — and until now nothing checked they still agree. An ABSENT
    # record is unknown, not different (the rule `an.bench.compare` applies to
    # an absent key), so this reads as an empty mapping and gates nothing.
    manifest = load_manifest(capture.name, chromium_build, root=root) or {}
    blessed_digests = {
        f.get("frame_key"): f.get("sha256") for f in (manifest.get("frames") or [])
    }
    ...
    for ref in refs:
        ...
        # `golden_sha256` and the bless record's `frames[].sha256` are the same
        # digest of the same file, written by two different calls, and a
        # disagreement means the committed PNG is not the picture a human
        # blessed: an edited file, a half-finished re-bless, a frame
        # regenerated by another encoder. Every one of those reads as a clean
        # PASS today. Compared on DECODED PIXELS, so a re-encode at a different
        # zlib level — Chromium 1187 -> 1223, 144 of 144 files, zero pixels —
        # is correctly invisible here.
        expected = blessed_digests.get(ref.key)
        record["bless_record_sha256"] = expected
        if expected is not None and expected != record["golden_sha256"]:
            record["state"] = "record_disagrees"
        frames.append(record)
    ...
    # After `unreadable` (a file that will not decode is the more basic
    # failure) and before `absent` (which is a gate, not an alarm): a
    # disagreement means an EXISTING golden is untrustworthy, which invalidates
    # the scene's whole verdict rather than merely leaving it un-set-up.
    # `unavailable`, not a new gate name: the check could not run, and a check
    # that could not run is not evidence anything is fine.
    disagreeing = [f for f in frames if f["state"] == "record_disagrees"]
    if disagreeing:
        return {
            "state": "unavailable",
            "detail": (
                "a committed golden's pixels disagree with its own bless "
                "record: "
                + "; ".join(
                    f"{f['frame_key']} decodes to {f['golden_sha256'][:12]}, "
                    f"the record says {f['bless_record_sha256'][:12]}"
                    for f in disagreeing
                )
                + f" (scene {capture.name!r}, Chromium {chromium_build}). The "
                "file is not the picture that was blessed, so comparing "
                "against it is not evidence of anything. Look at it, then "
                f'`an bench --scenes {capture.name} --bless "<why>"`.'
            ),
            "frames": frames,
        }
