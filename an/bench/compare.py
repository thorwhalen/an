...
def _verdict_under_mutation(prediction: dict, direction: str) -> str:
    ...
    Four outcomes rather than two, and the extra pair is the interesting half:

    - ``as_declared`` / ``contrary`` — it moved (or did not) as predicted.
    ...
    - ``unexpected_movement`` — a metric declared ``not_applicable`` or
      ``no_change`` **moved**. That is news, not a pass: a pre-encode statistic
      moving under an encoder mutation means the render changed too, and the
      mutation was not the only variable.
    """
    ...
    return "as_declared" if direction == expect else "contrary"
